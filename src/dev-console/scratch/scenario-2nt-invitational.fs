...
type private TwoNtInvitationalStrategy = | OldSchool2Nt | NewSchool2Nt | TooCoolForSchool with
    member this.Text =
        match this with
        | OldSchool2Nt -> "Old school (2NT invitational)" // 2NT (invitational) with 11-12; opener accepts with 14
        | NewSchool2Nt -> "New school (2NT invitational)" // 2NT (invitational) with 11-12; opener accepts with 14, or with 13 and a 5-card suit
            ...
            | Balanced, hcp when hcp >= 12<hcp> && hcp <= 14<hcp> -> // 1NT opener
                ...
                | Balanced, partnerHcp when (partnerHcp = 11<hcp> || partnerHcp = 12<hcp>) && partnerSpades < 4 && partnerHearts < 4 -> // balanced and invitational (combined strength of 23-26) with no 4-card or longer major
                    ...
                    if hcp = 14<hcp> && partnerHcp = 12<hcp> && partnerHand.Shape = FiveThreeThreeTwo then None
                        ...
                        let strategyContracts = // either 2NT (by opener) or 3NT (by opener) for OldSchool2Nt and NewSchool2Nt | either 1NT (by opener) or 3NT (by opener) for TooCoolForSchool
                            [
                                if hcp = 14<hcp> then
                                    OldSchool2Nt, undoubledContract ThreeLevel NoTrump position // 1NT | 2NT | 3NT | - (opener is declarer)
                                else OldSchool2Nt, undoubledContract TwoLevel NoTrump position // 1NT | 2NT | - (opener is declarer)
                                if hcp = 14<hcp> || (hcp = 13<hcp> && hand.Shape = FiveThreeThreeTwo) then
                                    NewSchool2Nt, undoubledContract ThreeLevel NoTrump position // 1NT | 2NT | 3NT | - (opener is declarer)
                                else NewSchool2Nt, undoubledContract TwoLevel NoTrump position // 1NT | 2NT | - (opener is declarer)
...
let run (mode:Mode) withDoubleDummy count =
    ...
    if withDoubleDummy then
        ...
        let oldSchool2NtMeanNotVulnerable, oldSchool2NtMeanVulnerable = Mean<score>.FromList(scores OldSchool2Nt NotVulnerable), Mean<score>.FromList(scores OldSchool2Nt Vulnerable)
        let newSchool2NtMeanNotVulnerable, newSchool2NtMeanVulnerable = Mean<score>.FromList(scores NewSchool2Nt NotVulnerable), Mean<score>.FromList(scores NewSchool2Nt Vulnerable)
        ...
        writeNewLine $"\t{OldSchool2Nt.Text} -> {oldSchool2NtMeanNotVulnerable.Mean} (non-vul.) | {oldSchool2NtMeanVulnerable.Mean} (vul.)" ConsoleColor.Gray
        writeNewLine $"\t{NewSchool2Nt.Text} -> {newSchool2NtMeanNotVulnerable.Mean} (non-vul.) | {newSchool2NtMeanVulnerable.Mean} (vul.)" ConsoleColor.Gray
